const botServer =
  "https://us-central1-zim-client.cloudfunctions.net/helloWorld/";
...
document.addEventListener(
  ...
  function() {
    ...
    if (document.body.contains(document.getElementById("zim-chatbot"))) {
      document.getElementById("zim-icon").addEventListener(
        "click",
        function(e) {
          showChat();
        },
      ...
      document.getElementById("send-chat-message").addEventListener(
        "click",
        function(e) {
      ...
      document
        .getElementById("text-chat")
        .addEventListener("keyup", function(event) {
          event.preventDefault();
          if (
            event.keyCode === 13 &&
            document.getElementById("text-chat").value.trim() != ""
          ) {
...
function toggleTheme() {
  ...
  setTimeout(function() {
    var color = window
      .getComputedStyle(document.getElementsByClassName("site-title")[0])
...
function sendChatMessage(message) {
  ...
  var serverWithMsg = botServer + message;
  fetch(serverWithMsg)
